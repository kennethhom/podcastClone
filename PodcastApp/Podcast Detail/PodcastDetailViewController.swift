...
class PodcastDetailViewController : UITableViewController {
    ...
    private func loadPodcast() {
        ...
        podcastFeedLoader.fetchFeed(lookup: podcastLookupInfo, priority: .userInitiated)
        
        cancellable = podcastFeedLoader.podcast.receive(on: DispatchQueue.main).sink(receiveCompletion: { completion in
            switch completion {
            case .finished:
                break
